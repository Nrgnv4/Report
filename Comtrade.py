...
class ComtradeRecord:
    ...
    def ReadCFG(self):
        ...
        # Read line and remove spaces and new line characters.
        ...
        for i in range(self.A):  # @UnusedVariable
            ...
            self.signals['Analog'].update({int(templist[0]):templist[1]})
        ...
        for i in range(self.D):  # @UnusedVariable
            line = self.filehandler.readline()
            templist = line.split(',')
            self.signals['Digital'].update({int(templist[0]):templist[1].encode('cp1251').decode('cp866')})
        ...
        self.trigger[5] = float(templist[2].replace('.-','.'))  # seconds.
    ...
    def getAnalogChannelData(self, ChNumber):
        ...
        str_struct = "ii%dh" % (
            self.A + int(numpy.ceil((float(self.D) / float(16)))))
        # Number of bytes per sample:
        NB = 4 + 4 + self.A * 2 + \
            int(numpy.ceil((float(self.D) / float(16)))) * 2
        # Number of samples:
        ...
        for i in range(N):
            data = struct.unpack(
                str_struct, self.DatFileContent[i * NB:(i * NB) + NB])
    ...
    def getDigitalChannelData(self, ChNumber):
        ...
        # Fomating string for struct module:
        str_struct = "ii%dh%dH" % (self.A, int(
            numpy.ceil((float(self.D) / float(16)))))
        # Number of bytes per sample:
        NB = 4 + 4 + self.A * 2 + \
            int(numpy.ceil((float(self.D) / float(16)))) * 2
        # Number of samples:
        ...
        # 16 digital channels:
        byte_number = int(numpy.ceil((ChNumber - 1) / 16))
        ...
        for i in range(N):
            ...
            values[i] = (digital_ch_value & data[self.A + 1 +\
                        byte_number]) * 1 / digital_ch_value
